...
//    }
    ...
    public void Diffusion2WithFlux(int bloodVesselDistance, double diffCoef){
        if(diffCoef>0.25){
            throw new IllegalArgumentException("Diffusion rate above stable maximum value of 0.25 value: "+diffCoef);
        }
        for (int x = 0; x < xDim; x++) {
            for (int y = 0; y < yDim; y++) {
                if (x!=0 && y!=0 && x!=xDim-1 && y!=yDim-1 && (x*yDim+y)%bloodVesselDistance==0 && (y*xDim+x)%bloodVesselDistance==0) {
                    Add(x,y,0.0);
                }
                Diffusion2(x,y, field, swapField, xDim, yDim, diffCoef, false, 0.0, wrapX, wrapY);
            }
    ...
    public void Diffusion2WithFlux(int bloodVesselDistance, double diffCoef, double boundaryValue){
        if(diffCoef>0.25){
            throw new IllegalArgumentException("Diffusion rate above stable maximum value of 0.25 value: "+diffCoef);
        }
        for (int x = 0; x < xDim; x++) {
