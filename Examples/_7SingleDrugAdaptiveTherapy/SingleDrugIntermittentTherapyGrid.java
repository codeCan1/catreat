...
public class SingleDrugIntermittentTherapyGrid extends AgentGrid2D<SingleDrugIntermittentTherapyCell> {
    //model constants
    ...
    public SingleDrugIntermittentTherapyGrid(int xDim, int yDim, Rand rn, double birthSensitiveProb, double birthResistantProb, double deathProb, double AT_Alpha, double AT_Beta, double Max_Tolerated_dose, double Min_Drug_Dose,double DRUG_ON_TIME, double DRUG_CYCLE_TIME ,double DRUG_DIFF_RATE,double DRUG_UPTAKE,double DRUG_DEATH,int check_tumorsize_interval_AT, double tumor_size_percent_triggering_AT, double replacement_threshold, double measurement_noise_sd, double Mut_SenToRes, double Mut_ResToSen, UILabel tickLabel,UILabel popLabel,UILabel drugLabel) {
        super(xDim, yDim, SingleDrugIntermittentTherapyCell.class);
        this.rn = rn;
    ...
    public static void main(String[] args) {
        ...
        SingleDrugIntermittentTherapyGrid[] models = new SingleDrugIntermittentTherapyGrid[4];
...
//        FileIO popsOut=new FileIO("Int2.csv","w");
//        BufferedReader userInput=new BufferedReader(new InputStreamReader(System.in));
        ...
        double AT_alpha=0.5;
        double AT_beta=0.1;
        double Max_Tolerated_Dose=5.0;
        double Min_Drug_Dose=0.5;
        double MTD_Reduction_Factor=0.75;
//        double MTD_Reduction_Factor=Double.parseDouble(args[3]);
        ...
        for (int i = 0; i < models.length; i++) {
            models[i]=new SingleDrugIntermittentTherapyGrid(x,y,new Rand(1),Div_Prob_Sen,Div_Prob_Res,deathProb,AT_alpha,AT_beta,Max_Tolerated_Dose, Min_Drug_Dose,DRUG_ON_TIME,DRUG_CYCLE_TIME,DRUG_DIFF_RATE,DRUG_UPTAKE,DRUG_DEATH,Check_Tumorsize_Interval_AT,Tumor_Size_Percent_Triggering_AT,Replacement_Threshold,Measurement_Noise_SD,Mut_SenToRes,Mut_ResToSen,tickLabel,popLabel,drugLabel);
...
//        models[2].MAX_TOLERATED_DOSE = models[2].MAX_TOLERATED_DOSE/10; //metronomic therapy uses 1/10 drug
...
//            }
    ...
    public void ModelStep(int tick) {
        ...
        ShuffleAgents(rn);
        for (SingleDrugIntermittentTherapyCell cell : this) {
            cell.CellStep();
        }
...
//                drugLabel.SetText("Current Drug Metronomic is "+String.format("%.3f",CURRENT_DRUG)+"       Grid Avg is "+String.format("%.3f",drug.GetAvg()));
        ...
        for (int i = 0; i < length; i++) {
            SingleDrugIntermittentTherapyCell drawMe = GetAgent(i);
...
class SingleDrugIntermittentTherapyCell extends AgentSQ2Dunstackable<SingleDrugIntermittentTherapyGrid> {
...
//    public double drug_sensitivity_score;
    ...
    public void DivideAndMutate(int index) {
        SingleDrugIntermittentTherapyCell tmp = G.NewAgentSQ(index);
...
//                System.out.println("tmpAgentIndex is " + tmpAgentIndex);
                SingleDrugIntermittentTherapyCell tmpAgent = G.GetAgent(tmpAgentIndex);
