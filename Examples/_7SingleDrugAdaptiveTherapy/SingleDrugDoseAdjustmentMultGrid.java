...
public class SingleDrugDoseAdjustmentMultGrid extends AgentGrid2D<SingleDrugDoseAdjustmentMultCell> {
    //model constants
    ...
    public double AT_ALPHA, AT_GAMMA, AT_BETA, MAX_TOLERATED_DOSE, MIN_DRUG_DOSE;
    ...
    public static void main(String[] args) {
        ...
        SingleDrugDoseAdjustmentMultGrid[] models = new SingleDrugDoseAdjustmentMultGrid[4];
...
//        FileIO popsOut=new FileIO("DosMod2.csv","w");
        ...
        double AT_alpha=0.5;
        double AT_gamma=2;
//        double AT_gamma=Double.parseDouble(args[3]);
        double AT_beta=0.1;
...
//            }
    ...
    public void ModelStep(int tick) {
        ...
        ShuffleAgents(rn);
        for (SingleDrugDoseAdjustmentMultCell cell : this) {
            cell.CellStep();
        }
...
//                drugLabel.SetText("Current Drug Metronomic is "+String.format("%.3f",CURRENT_DRUG)+"       Grid Avg is "+String.format("%.3f",drug.GetAvg()));
        ...
        for (int i = 0; i < length; i++) {
            SingleDrugDoseAdjustmentMultCell drawMe = GetAgent(i);
...
class SingleDrugDoseAdjustmentMultCell extends AgentSQ2Dunstackable<SingleDrugDoseAdjustmentMultGrid> {
...
//    public double drug_sensitivity_score;
    ...
    public void DivideAndMutate(int index) {
        SingleDrugDoseAdjustmentMultCell tmp = G.NewAgentSQ(index);
...
//                System.out.println("tmpAgentIndex is " + tmpAgentIndex);
                SingleDrugDoseAdjustmentMultCell tmpAgent = G.GetAgent(tmpAgentIndex);
