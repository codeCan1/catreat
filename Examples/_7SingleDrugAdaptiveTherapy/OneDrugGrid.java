...
public class OneDrugGrid extends AgentGrid2D<OneDrugCell> {
    //model constants
    ...
    public static void main(String[] args) {
        ...
        OneDrugGrid[] models = new OneDrugGrid[4];
//        String my_strategy="DosAdjSdAdd-DelDose-0.25-MTD_RF-1-MTD-5";
        String my_strategy="DosAdjSdAdd-DelDose-0.5-MTD_RF-1-MTD-5";
//        String my_strategy="DosAdjSdMult-DelDose-1.5-MTD_RF-1-MTD-5";
//        String my_strategy="DosAdjSdMult-DelDose-2-MTD_RF-1-MTD-5";


//        String my_strategy="IntFixedDoseSd-DelDose-0-MTD_RF-0.75-MTD-5";
//        String my_strategy="IntFixedDoseSd-DelDose-0-MTD_RF-1-MTD-5";

//        String my_strategy="HolFixedDoseSd-DelDose-0-MTD_RF-0.75-MTD-5";
//        String my_strategy="HolFixedDoseSd-DelDose-0-MTD_RF-1-MTD-5";



        String[] my_strategy_params=my_strategy.split("-");
        String file_counter = "1";
//        FileIO popsOut=new FileIO("DosMod2.csv","w");
        FileIO popsOut=new FileIO(my_strategy+"-"+file_counter,"w");
...
//        The defaults are Div Sen:0.08(0.025) Div Res:0.02(0.0011) Death Rate:0.01(0.0)
        double Div_Prob_Sen=0.04;
        double Div_Prob_Res=0.01;
        double deathProb=0.001;
        double AT_alpha=Double.parseDouble(my_strategy_params[2]);
        double AT_gamma=Double.parseDouble(my_strategy_params[2]);
        double AT_beta=0.1;
        double Max_Tolerated_Dose=Double.parseDouble(my_strategy_params[6]);
        double Min_Drug_Dose=0.5;
        double MTD_Reduction_Factor=Double.parseDouble(my_strategy_params[4]);
...
//            models[i]=new OneDrugGrid(x,y,new Rand(1),Div_Prob_Sen,Div_Prob_Res,deathProb,AT_alpha,AT_gamma, AT_beta,Max_Tolerated_Dose, Min_Drug_Dose,DRUG_ON_TIME,DRUG_CYCLE_TIME,DRUG_DIFF_RATE,DRUG_UPTAKE,DRUG_DEATH,Check_Tumorsize_Interval_AT,Tumor_Size_Percent_Triggering_AT,Replacement_Threshold,Measurement_Noise_SD,Mut_SenToRes,Mut_ResToSen,tickLabel,popLabel,drugLabel);
        ...
        models[3].MAX_TOLERATED_DOSE=Max_Tolerated_Dose*MTD_Reduction_Factor;
...
//        models[2].DRUG_CYCLE_TIME=4.0;
        ...
        for (int tick = 0; tick < 5000; tick++) {
            ...
            for (int i = 0; i < models.length; i++) {
                if (i==0||i==1||i==2) {
                    ...
                    models[i].ModelStepAdaptiveTherapy(tick,my_strategy_params[0]);
                }
            ...
            //data recording
            popsOut.Write(tick+","+my_strategy+","+models[0].Pop()+","+models[0].SEN_POP+","+models[0].RES_POP+","+models[1].Pop()+","+models[1].SEN_POP+","+models[1].RES_POP+","+models[2].Pop()+","+models[2].SEN_POP+","+models[2].RES_POP+","+models[3].Pop()+","+models[3].SEN_POP+","+models[3].RES_POP+","+models[3].CURRENT_DRUG+",");
    ...
    public void ModelStepAdaptiveTherapy(int tick, String my_strategy) {

        AdaptiveTherapyChecker(tick,my_strategy);
...
//        System.out.println("Tumor measurement after noise is " +CURRENT_POP);
    ...
    public void AdaptiveTherapyChecker(int tick, String strategy) {
...
//                    CURRENT_DRUG=((1+AT_ALPHA)*PREV_DRUG)>=MAX_TOLERATED_DOSE?MAX_TOLERATED_DOSE:((1+AT_ALPHA)*PREV_DRUG);
                            CURRENT_DRUG=(AT_GAMMA*PREV_DRUG)>=MAX_TOLERATED_DOSE?MAX_TOLERATED_DOSE:((AT_GAMMA)*PREV_DRUG);
                            System.out.println("Tick: "+tick+" Current Drug for CurrentPop "+CURRENT_POP+" >(1+at_beta)*Prev Pop "+PREV_POP+" is "+CURRENT_DRUG);
                            PREV_POP=CURRENT_POP;
                            PREV_DRUG=CURRENT_DRUG;
                            IS_TREATMENT_VACATION_ON=false;
...
//                    CURRENT_DRUG=((1-AT_ALPHA)*PREV_DRUG)<=MIN_DRUG_DOSE?MIN_DRUG_DOSE:(1-AT_ALPHA)*PREV_DRUG;
                            CURRENT_DRUG=(PREV_DRUG/AT_GAMMA)<=MIN_DRUG_DOSE?MIN_DRUG_DOSE:(PREV_DRUG/AT_GAMMA);
                            System.out.println("Tick: "+tick+" Current Drug for CurrentPop "+CURRENT_POP+" <=(1-at_beta)*Prev Pop "+PREV_POP+" is "+CURRENT_DRUG);
                            PREV_POP=CURRENT_POP;
                            PREV_DRUG=CURRENT_DRUG;
                            IS_TREATMENT_VACATION_ON=false;
                ...
                if (HAS_AT_STARTED) {
                    ...
                    if ((tick-AT_START_TICK)%CHECK_TUMORSIZE_INTERVAL_AT==0) {
                        ...
                        if(IS_IT_UP_PHASE) {
                            if (CURRENT_POP<TUMOR_SIZE_TRIGGERING_AT) {
                                CURRENT_DRUG=0.0;
                                System.out.println("Tick: "+tick+" Current Drug for CurrentPop "+CURRENT_POP+"is "+CURRENT_DRUG);
                                return;
                            }
                        }
...
class OneDrugCell extends AgentSQ2Dunstackable<OneDrugGrid> {
...
//    public double drug_sensitivity_score;
    ...
    public void DivideAndMutate(int index) {
        OneDrugCell tmp = G.NewAgentSQ(index);
...
//        Mutate();
        ...
        if (G.rn.Double(1) < tmp_div_prob) {
            ...
            else if (options_empty==0 && G.rn.Double(1)<G.REPLACEMENT_THRESHOLD) { //&& this.type==G.RESISTANT) {
...
//                System.out.println("tmpAgentIndex is " + tmpAgentIndex);
                OneDrugCell tmpAgent = G.GetAgent(tmpAgentIndex);
