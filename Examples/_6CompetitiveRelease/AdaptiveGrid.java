...
public class AdaptiveGrid extends AgentGrid2D<AdaptiveCell> {
    //model constants
    ...
    public static void main(String[] args) {
        ...
        AdaptiveGrid[] models = new AdaptiveGrid[4];
        FileIO popsOut=new FileIO("RT5.csv","w");
        ...
        double Replacement_Threshold=0.0000001;
        double Measurement_Noise_SD=0;

        for (int i = 0; i < models.length; i++) {
            models[i]=new AdaptiveGrid(x,y,new Rand(1),Div_Prob_Sen,Div_Prob_Res,deathProb,AT_alpha,AT_beta,Max_Tolerated_Dose, Min_Drug_Dose,Check_Tumorsize_Interval_AT,Tumor_Size_Percent_Triggering_AT,Replacement_Threshold,Measurement_Noise_SD);
...
//        }
    ...
    public void DrawModel(UIGrid vis, int iModel) {
        for (int i = 0; i < length; i++) {
            AdaptiveCell drawMe = GetAgent(i);
...
class AdaptiveCell extends AgentSQ2Dunstackable<AdaptiveGrid> {
...
//                System.out.println("tmpAgentIndex is " + tmpAgentIndex);
                AdaptiveCell tmpAgent = G.GetAgent(tmpAgentIndex);
