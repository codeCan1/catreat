...
//This script simulates adaptive therapy with resistance being binary phenotype (0 or 1)
//March 2019
...
public class ATBinaryModel extends AgentGrid2D<ATBinaryCell> {
    //model constants
    ...
    public ATBinaryModel(int xDim, int yDim, Rand rn, double birthSensitiveProb, double birthResistantProb, double deathProb, double AT_Alpha, double AT_Beta, double Max_Tolerated_dose, double Min_Drug_Dose, int check_tumorsize_interval_AT, double tumor_size_percent_triggering_AT) {
        super(xDim, yDim, ATBinaryCell.class);
        this.rn = rn;
    ...
    public static void main(String[] args) {
        ...
        ATBinaryModel[] models = new ATBinaryModel[4];
        FileIO popsOut=new FileIO("AT17.csv","w");
        ...
        for (int i = 0; i < models.length; i++) {
            models[i]=new ATBinaryModel(x,y,new Rand(1),Div_Prob_Sen,Div_Prob_Res,deathProb,AT_alpha,AT_beta,Max_Tolerated_Dose, Min_Drug_Dose,Check_Tumorsize_Interval_AT,Tumor_Size_Percent_Triggering_AT);
...
//        models[2].DRUG_CYCLE_TIME = 200;
        ...
        for (int tick = 0; tick < 10000; tick++) {
            ...
            //data recording
            popsOut.Write(models[0].Pop()+","+models[0].SEN_POP+","+models[0].RES_POP+","+models[1].Pop()+","+models[1].SEN_POP+","+models[1].RES_POP+","+models[2].Pop()+","+models[2].SEN_POP+","+models[2].RES_POP+","+models[3].Pop()+","+models[3].SEN_POP+","+models[3].RES_POP+"\n");
//            if((tick)%100==0) {
...
//        if(IS_AT_ON) {
//            for(int i=0;i<(int)DRUG_METABOLISM_TIME;i++) {
//                drug.DiffusionADI(DRUG_DIFF_RATE,CURRENT_DRUG);
//            }
//            IS_AT_ON=false;
//        }

//        if(IS_AT_ON && IS_TREATMENT_VACATION_ON) {
//            for(int i=0;i<(int)DRUG_METABOLISM_TIME;i++) {
//                drug.DiffusionADI(DRUG_DIFF_RATE);
//            }
//            drug.DiffusionADI(DRUG_DIFF_RATE);
...
//        }
    ...
    public void DrawModel(UIGrid vis, int iModel) {
        for (int i = 0; i < length; i++) {
            ATBinaryCell drawMe = GetAgent(i);
        ...
        if (HAS_AT_STARTED) {
            ...
            if ((tick-AT_START_TICK)%CHECK_TUMORSIZE_INTERVAL_AT==0) {
                ...
                if(CURRENT_POP<=(0.5*TUMOR_SIZE_TRIGGERING_AT)) {
                    ...
                    PREV_POP=CURRENT_POP;
                    PREV_DRUG=(PREV_DRUG<=MIN_DRUG_DOSE)?MIN_DRUG_DOSE:0.5*PREV_DRUG;
                    IS_AT_ON=true;
                    return true;
                }
...
class ATBinaryCell extends AgentSQ2Dunstackable<ATBinaryModel> {
