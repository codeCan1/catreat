...
public class CkTandemAddGrid extends AgentGrid2D<CkTandemAddCell> {
    //model constants
    ...
    public int PREV_POP, CURRENT_POP,LAST_MAXIMUM_POP;
...
//                    break;
        ...
        for (int i = 0; i < length; i++) {
            CkTandemAddCell drawMe = GetAgent(i);
...
//    }
    ...
    public void StandardTherapyChecker(int tick) {
        CURRENT_POP = Pop();
...
//        System.out.println("Tumor measurement after noise is " +CURRENT_POP);
        ...
        if(!HAS_AT_STARTED) {
            ...
            if(CURRENT_POP>=TUMOR_SIZE_TRIGGERING_AT) {
                ...
                System.out.println("Tick: "+tick+" Current Drug A and B for first dose and Current Pop "+CURRENT_POP+
                        " >= Tumor size triggering AT is "+CURRENT_DRUG_A+" "+ CURRENT_DRUG_B+". Prev pop is "+PREV_POP);
                AT_START_TICK=tick;
                LAST_MAXIMUM_POP=CURRENT_POP;
...
//                VARIABLE_BETA=LAST_MAXIMUM_POP>CURRENT_POP?LAST_MAXIMUM_POP-CURRENT_POP:0;
...
//                    IS_AT_ON=true;
                ...
                if(CURRENT_POP>((int)((1-AT_BETA)*PREV_POP)) && CURRENT_POP<=((int)((1+AT_BETA)*PREV_POP))) {
                    if (CURRENT_POP<LAST_MAXIMUM_POP) {
...
class CkTandemAddCell extends AgentSQ2Dunstackable<CkTandemAddGrid> {
...
//    public double drug_sensitivity_score;
    ...
    public void DivideAndMutate(int index) {
        CkTandemAddCell tmp = G.NewAgentSQ(index);
...
//        Mutate();
        ...
        if (G.rn.Double() < tmp_div_prob) {
            ...
            if (options_empty==0 && G.rn.Double()<G.REPLACEMENT_THRESHOLD) { //&& this.type==G.DOUBLY_RESISTANT) {
                ...
                CkTandemAddCell tmpAgent = G.GetAgent(tmpAgentIndex);
