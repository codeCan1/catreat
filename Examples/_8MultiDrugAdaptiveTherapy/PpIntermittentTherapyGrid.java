...
public class PpIntermittentTherapyGrid extends AgentGrid2D<PpIntermittentTherapyCell> {
    //model constants
...
//    public double DRUG_ON_TIME = 40, DRUG_OFF_TIME = 160, DRUG_CYCLE_TIME = 200, DRUG_DIFF_RATE = 2.0, DRUG_UPTAKE = 0.91, DRUG_BOUNDARY_VAL = 1.0, DRUG_METABOLISM_TIME = 40.0, DRUG_A_DEATH=0.2, DRUG_B_DEATH=0.2;
    ...
    public double AT_ALPHA_DRUG_A, AT_ALPHA_DRUG_B,AT_BETA, MAX_TOLERATED_DOSE_A, MAX_TOLERATED_DOSE_B, MIN_DRUG_DOSE_A, MIN_DRUG_DOSE_B;
    public double REPLACEMENT_THRESHOLD;
    public double MEASUREMENT_NOISE_SD;
    public double Mut_DoublySen_to_ResA = 1e-3;
    public double Mut_DoublySen_to_ResB = 1e-3;
    public double Mut_ResA_to_DoublyRes = 1e-3;
    public double Mut_ResB_to_DoublyRes = 1e-3;
    public double Mut_DoublySen_to_DoublyRes = 1e-3;

    public double Mut_DoublyRes_to_ResA = 1e-3;
    public double Mut_DoublyRes_to_ResB = 1e-3;
    public double Mut_DoublyRes_to_DoublySen = 1e-3;
    public double Mut_ResA_to_DoublySen = 1e-3;
    public double Mut_ResB_to_DoublySen = 1e-3;
    ...
    public boolean IS_IT_DOWN_PHASE;
    public boolean IS_IT_UP_PHASE;
    public boolean IS_LAST_DRUG_DELIVERED_A;
    public boolean IS_LAST_DRUG_DELIVERED_B;
    ...
    public static void main(String[] args) {
        ...
        PpIntermittentTherapyGrid[] models = new PpIntermittentTherapyGrid[4];
        String strategy = "Int";
        String sub_strategy = "PpHiDrug";
...
//        BufferedReader userInput=new BufferedReader(new InputStreamReader(System.in));
        ...
        double MTD_Reduction_Factor_DrugA=0.35;
        double MTD_Reduction_Factor_DrugB=0.35;
//        double MTD_Reduction_Factor_DrugA=1.0;
//        double MTD_Reduction_Factor_DrugB=1.0;
...
//            models[i]=new PpIntermittentTherapyGrid(x,y,new Rand(1),Div_Prob_Sen,Div_Prob_Res,deathProb,AT_alpha_drug_A,AT_alpha_drug_B,AT_beta,Max_Tolerated_Dose_A,Max_Tolerated_Dose_B, Min_Drug_Dose_A,Min_Drug_Dose_B,DRUG_ON_TIME,DRUG_CYCLE_TIME,DRUG_DIFF_RATE,DRUG_UPTAKE,DRUG_A_DEATH,DRUG_B_DEATH,Check_Tumorsize_Interval_AT,Tumor_Size_Percent_Triggering_AT,Replacement_Threshold,Measurement_Noise_SD);
        ...
        models[3].MAX_TOLERATED_DOSE_A=Max_Tolerated_Dose_A*MTD_Reduction_Factor_DrugA;
        models[3].MAX_TOLERATED_DOSE_B=Max_Tolerated_Dose_B*MTD_Reduction_Factor_DrugB;
...
//
//        ShuffleAgents(rn);
//        for (PpIntermittentTherapyCell cell : this) {
//            cell.CellStep();
//        }
//
//        //check if drugA should enter through the boundaries
////        if (Pop()>TUMOR_SIZE_TRIGGERING_AT) {
////            if (tick > DRUG_START && (tick - DRUG_START) % DRUG_OFF_TIME < DRUG_ON_TIME) {
////                drugA.DiffusionADI(DRUG_DIFF_RATE, DRUG_BOUNDARY_VAL);
////            } else {
////                drugA.DiffusionADI(DRUG_DIFF_RATE);
////            }
////        }
//    }
    public void ModelStep(int tick) {
...
//        drugA.MulAll(0.956); //first order drug decay kinetics for Tamoxifen per hour calculated after finding elimination rate constant lambda. Lambda is Clearance (CL) over Volume of Distribution (Vd).Vd is 55L/kg and CL is 2.5L/hour. Then C=C0exp(-lambda*t) where t is 1 hour
//        drugB.MulAll(0.956);
//
//        drugA.DiffusionADI(drug_diffusion_rate);
//        drugB.DiffusionADI(drug_diffusion_rate);
...
//                    break;
        ...
        for (int i = 0; i < length; i++) {
            PpIntermittentTherapyCell drawMe = GetAgent(i);
...
//        System.out.println("Tumor measurement after noise is " +CURRENT_POP);
        ...
        if(!HAS_AT_STARTED) {
            ...
            if(CURRENT_POP>=TUMOR_SIZE_TRIGGERING_AT) {
                HAS_AT_STARTED=true;
                IS_IT_DOWN_PHASE=true;
                IS_IT_UP_PHASE=false;
                CURRENT_DRUG_A=MAX_TOLERATED_DOSE_A;
                CURRENT_DRUG_B=0.0;
                IS_LAST_DRUG_DELIVERED_A=true;
                IS_LAST_DRUG_DELIVERED_B=false;
                System.out.println("Tick: "+tick+" Current Drug A and B for first dose and Current Pop "+CURRENT_POP+" >= Tumor size triggering AT is "+CURRENT_DRUG_A+" "+ CURRENT_DRUG_B);
                AT_START_TICK=tick;
            ...
            if ((tick-AT_START_TICK)%CHECK_TUMORSIZE_INTERVAL_AT==0) {
                ...
                if(IS_IT_UP_PHASE) {
                    if (CURRENT_POP<TUMOR_SIZE_TRIGGERING_AT) {
                        CURRENT_DRUG_A=0.0;
                        CURRENT_DRUG_B=0.0;
                        System.out.println("Tick: "+tick+" Current Drug for CurrentPop "+CURRENT_POP+"is "+CURRENT_DRUG_A+" and"+CURRENT_DRUG_B+" respectively");
                        return true;
                    }
                }

                if(IS_IT_DOWN_PHASE) {
                    if (IS_LAST_DRUG_DELIVERED_A) {
                        CURRENT_DRUG_A=0.0;
                        CURRENT_DRUG_B=MAX_TOLERATED_DOSE_B;
                        System.out.println("Tick: "+tick+" Current Drug for CurrentPop "+CURRENT_POP+"is "+CURRENT_DRUG_A+" and"+CURRENT_DRUG_B+" respectively");
                        IS_LAST_DRUG_DELIVERED_B=true;
                        IS_LAST_DRUG_DELIVERED_A=false;
                    }
                    else if(IS_LAST_DRUG_DELIVERED_B) {
                        CURRENT_DRUG_B=0.0;
                        CURRENT_DRUG_A=MAX_TOLERATED_DOSE_A;
                        System.out.println("Tick: "+tick+" Current Drug for CurrentPop "+CURRENT_POP+"is "+CURRENT_DRUG_A+" and"+CURRENT_DRUG_B+" respectively");
                        IS_LAST_DRUG_DELIVERED_B=false;
...
//                    CURRENT_DRUG_A=0.0;
//                    CURRENT_DRUG_B=0.0;
//                    System.out.println("Tick: "+tick+" Current Drug A and B for CurrentPop "+CURRENT_POP+" <=0.5 of the tumor size triggering AT is "+CURRENT_DRUG_A+" "+CURRENT_DRUG_B+". Prev pop is "+PREV_POP);
//                    PREV_POP=CURRENT_POP;
...
//                    CURRENT_DRUG_A=MAX_TOLERATED_DOSE_A;
//                    CURRENT_DRUG_B=MAX_TOLERATED_DOSE_B;
//                    System.out.println("Tick: "+tick+" Current Drug A and B for pop "+CURRENT_POP+" >0.95*carrying capacity(xDim*yDim)  "+CURRENT_DRUG_A+" "+CURRENT_DRUG_B+". Prev pop is "+PREV_POP);
//                    PREV_POP=CURRENT_POP;
//                    PREV_DRUG_A=CURRENT_DRUG_A;
//                    PREV_DRUG_B=CURRENT_DRUG_B;
//                    IS_AT_ON=true;
...
//                    System.out.println("Tick: "+tick+" Current Drug A and B for CurrentPop "+CURRENT_POP+" >(1+at_beta)*Prev Pop "+PREV_POP+" is "+CURRENT_DRUG_A+" "+CURRENT_DRUG_B);
//                    PREV_POP=CURRENT_POP;
//                    PREV_DRUG_A=CURRENT_DRUG_A;
//                    PREV_DRUG_B=CURRENT_DRUG_B;
//                    IS_AT_ON=true;
...
//                        System.out.println("Tick: "+tick+" Current Drug A for Current Pop "+CURRENT_POP+" within the threshold(beta) of prev pop "+PREV_POP+" is "+CURRENT_DRUG_A);
//                        System.out.println("Tick: "+tick+" Current Drug B for Current Pop "+CURRENT_POP+" within the threshold(beta) of prev pop "+PREV_POP+" is "+CURRENT_DRUG_B);
//                        PREV_POP=CURRENT_POP;
//                        PREV_DRUG_A=CURRENT_DRUG_A;
//                        PREV_DRUG_B=CURRENT_DRUG_B;
////                        IS_AT_ON=true;
...
//                    CURRENT_DRUG_A=PREV_DRUG_A;
//                    CURRENT_DRUG_B=PREV_DRUG_B;
//                    System.out.println("Tick: "+tick+" Current Drug A and B for Current Pop "+CURRENT_POP+" within the threshold(beta) of prev pop "+PREV_POP+" is "+CURRENT_DRUG_A+" "+CURRENT_DRUG_B);
//                    PREV_POP=CURRENT_POP;
//                    PREV_DRUG_A=CURRENT_DRUG_A;
//                    PREV_DRUG_B=CURRENT_DRUG_B;
//                    IS_AT_ON=true;
...
class PpIntermittentTherapyCell extends AgentSQ2Dunstackable<PpIntermittentTherapyGrid> {
...
//    public double drug_sensitivity_score;
    ...
    public void DivideAndMutate(int index) {
        PpIntermittentTherapyCell tmp = G.NewAgentSQ(index);
...
//                System.out.println("tmpAgentIndex is " + tmpAgentIndex);
                PpIntermittentTherapyCell tmpAgent = G.GetAgent(tmpAgentIndex);
