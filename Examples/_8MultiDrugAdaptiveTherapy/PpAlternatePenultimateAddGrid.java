...
public class PpAlternatePenultimateAddGrid extends AgentGrid2D<PpAlternatePenultimateAddCell> {
    //model constants
    ...
    public boolean IS_LAST_DRUG_ADJUSTED_A, IS_LAST_DRUG_ADJUSTED_B,IS_IT_FIRST_TIME_DRUG_A,HAVE_WE_USED_DRUG_B_BEFORE;
    ...
    public static void main(String[] args) {
        ...
        PpAlternatePenultimateAddGrid[] models = new PpAlternatePenultimateAddGrid[4];
...
//        StandardTherapyChecker(tick);
        ...
        ShuffleAgents(rn);
        for (PpAlternatePenultimateAddCell cell : this) {
            cell.CellStep();
        }
    }
...
//                    break;
        ...
        for (int i = 0; i < length; i++) {
            PpAlternatePenultimateAddCell drawMe = GetAgent(i);
        ...
        if (HAS_AT_STARTED) {
            ...
            if ((tick - AT_START_TICK) % CHECK_TUMORSIZE_INTERVAL_AT == 0) {
                ...
                if (IS_LAST_DRUG_ADJUSTED_A) {
                    ...
                    if(!HAVE_WE_USED_DRUG_B_BEFORE) {
                        CURRENT_DRUG_B=MAX_TOLERATED_DOSE_B;
                        CURRENT_DRUG_A=0.0;
                        HAVE_WE_USED_DRUG_B_BEFORE=true;
...
class PpAlternatePenultimateAddCell extends AgentSQ2Dunstackable<PpAlternatePenultimateAddGrid> {
...
//    public double drug_sensitivity_score;
    ...
    public void DivideAndMutate(int index) {
        PpAlternatePenultimateAddCell tmp = G.NewAgentSQ(index);
...
//                System.out.println("tmpAgentIndex is " + tmpAgentIndex);
                PpAlternatePenultimateAddCell tmpAgent = G.GetAgent(tmpAgentIndex);
