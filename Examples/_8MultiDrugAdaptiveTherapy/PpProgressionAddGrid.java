...
public class PpProgressionAddGrid extends AgentGrid2D<PpProgressionAddCell> {
    //model constants
...
//    public double PERCENT_INCREMENT_DRUG_LAST_CYCLE=0.5;
    ...
    public static void main(String[] args) {
        ...
        PpProgressionAddGrid[] models = new PpProgressionAddGrid[4];
        String strategy = "DosAdjAdd50";
        String sub_strategy = "PpProg";
        String file_counter = "HighTurnover3";
...
//            int tmp = rn.Double() < resistantProb?rn.Double()<resistantProb?DOUBLY_RESISTANT:RESISTANT_TO_DRUG_A:rn.Double()<resistantProb?DOUBLY_SENSITIVE:RESISTANT_TO_DRUG_B;
    ...
    public void ModelStep(int tick) {
        ...
        ShuffleAgents(rn);
        for (PpProgressionAddCell cell : this) {
            cell.CellStep();
        }
    }
...
//                    break;
        ...
        for (int i = 0; i < length; i++) {
            PpProgressionAddCell drawMe = GetAgent(i);
...
//                }
                if(CURRENT_POP>((int)(((1+AT_BETA)*PREV_POP)))) {
                    if(!IS_IT_FIRST_TIME_DRUG_B) {
                        ...
                        System.out.println("Tick: "+tick+" Current Drug A and B for CurrentPop "+CURRENT_POP+" is "+CURRENT_DRUG_A+" "+CURRENT_DRUG_B+". Prev pop is "+PREV_POP);
                        IS_LAST_DRUG_ADJUSTED_B=true;
                        IS_LAST_DRUG_ADJUSTED_A=false;
                        PREV_DRUG_B=CURRENT_DRUG_B;
                        PREV_POP=CURRENT_POP;
...
class PpProgressionAddCell extends AgentSQ2Dunstackable<PpProgressionAddGrid> {
...
//    public double drug_sensitivity_score;
    ...
    public void DivideAndMutate(int index) {
        PpProgressionAddCell tmp = G.NewAgentSQ(index);
...
//                System.out.println("tmpAgentIndex is " + tmpAgentIndex);
                PpProgressionAddCell tmpAgent = G.GetAgent(tmpAgentIndex);
